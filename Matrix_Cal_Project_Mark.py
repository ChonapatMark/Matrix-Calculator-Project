...
def select_argument_M():
    ...
    if argumant_M == "y" or argumant_M == "Y":
        ...
        Start_M = None  # Add to handle invalid input
    return Start_M
...
def elementary_op(A):
    row, col = A.shape
    for i in range(row):  # Loop through each pivot row
        # Make sure the diagonal element is not zero to avoid division by zero
        if A[i, i] == 0:
            print(f"Error: Zero pivot found at row {i+1}. Row swapping needed, but not implemented.")
            return None
        
        for j in range(i + 1, row):  # Loop through each row below the pivot row
            E = np.eye(row)  # Initialize identity matrix for each elementary operation
            factor = -(A[j, i] / A[i, i])  # Compute the factor to eliminate A[j, i]
            E[j, i] = factor  # Set the appropriate value in the elementary matrix
...
# Function to convert matrix to RREF form
def rref(A):
    row, col = A.shape
    for i in range(row):  # Loop through each pivot row
        # Step 1: Make sure the pivot is 1
        if A[i, i] != 1:
            E = np.eye(row)
            factor = 1 / A[i, i]
            E[i, i] = factor
        ...
        # Step 2: Eliminate above and below the pivot
        for j in range(row):
            if j != i:
                E = np.eye(row)
                factor = -A[j, i]
...
if response :  # Check if response is not None before printing
